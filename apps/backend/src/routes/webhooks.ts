...
export async function webhookRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/:id", {
    ...
    try {
      ...
      const webhook = await Webhook.findOne({ _id: id, tenantId: tenant._id })
        .select('-secretKey');
  ...
  fastify.post("/", {
    ...
    try {
      ...
      const webhook = new Webhook({
        ...
        apiVersion: webhookData.apiVersion || "2024-01-01",
        secretKey,
      ...
      reply.status(201).send({
        ...
        message: "Webhook created successfully. Store the secret key securely - it won't be shown again.",
      });
    } catch (error: any) {
      reply.status(500).send({
        success: false,
  ...
  fastify.delete("/:id", {
    ...
    try {
      ...
      const webhook = await Webhook.findOneAndDelete({ _id: id, tenantId: tenant._id });
  ...
  fastify.post("/:id/regenerate-secret", {
    ...
    try {
      ...
      reply.send({
        ...
        message: "Secret key regenerated successfully. Update your webhook endpoint to use the new secret.",
      });
    } catch (error: any) {
      reply.status(500).send({
        success: false,

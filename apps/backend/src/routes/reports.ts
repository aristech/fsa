    ...
    try {
      ...
      if (reportData.taskIds && reportData.taskIds.length > 0) {
        const tasks = await Task.find({
          _id: { $in: reportData.taskIds },
          tenantId: user.tenantId,
        });
        if (tasks.length !== reportData.taskIds.length) {
          return reply
            .code(400)
            .send({ success: false, message: "One or more tasks not found" });
        }
      }
      ...
      if (reportData.taskIds && reportData.taskIds.length > 0) {
        ...
        // Convert TimeEntry format to ReportTimeEntry format
        for (const timeEntry of timeEntries) {
          const reportTimeEntry = {
            description: timeEntry.notes || `Time logged on task`,
            startTime: timeEntry.date, // Use date as start time
            endTime: new Date(
              timeEntry.date.getTime() + timeEntry.hours * 60 * 60 * 1000,
            ), // Calculate end time
            duration: timeEntry.hours * 60, // Convert hours to minutes
            taskId: timeEntry.taskId,
            category: "labor" as const,
          };
          report.timeEntries.push(reportTimeEntry as any);
        }

        // Calculate total labor cost from time entries
        report.totalLaborCost = timeEntries.reduce(
    ...
    async (request, reply) => {
      try {
        ...
        if (!reason) {
          return reply
            .code(400)
            .send({ success: false, message: "Rejection reason is required" });
        }
    ...
    async (request, reply) => {
      try {
        ...
        if (!material) {
          return reply
            .code(404)
            .send({ success: false, message: "Material not found" });
        }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        const { id } = request.params as { id: string };
        const user = (request as AuthenticatedRequest).user;
        const { description, startTime, endTime, taskId, category } =
          request.body as {
            description: string;
            startTime: string;
            endTime: string;

...
export class RecurringTaskService {
  ...
  static async updateTaskRecurrence(taskId: string): Promise<void> {
    ...
    const nextOccurrence = this.calculateNextOccurrence(task.dueDate, task.repeat);
   ...
   */
  static async createRecurringInstance(originalTask: ITask): Promise<ITask | null> {
    ...
    const originalStartDate = originalTask.startDate ? dayjs(originalTask.startDate) : null;

    const newDueDate = dayjs(this.calculateNextOccurrence(originalTask.dueDate, originalTask.repeat));
    ...
    const newTaskData = {
      ...
      reminder: originalTask.reminder?.enabled ? originalTask.reminder : undefined,
   ...
   */
  static async processPendingRecurringTasks(): Promise<{ processed: number; errors: string[] }> {
    ...
    for (const task of tasks) {
      try {
        console.log(`Processing recurring task "${task.name}" (${task._id})`);
        ...
        if (newTask) {
          ...
          processed++;
          console.log(`✅ Created recurring instance of "${task.name}" with ID: ${newTask._id}`);
        } else {
          console.log(`⚠️ Could not create recurring instance for task "${task.name}"`);
        }
      } catch (error) {

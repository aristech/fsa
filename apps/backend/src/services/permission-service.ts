...
export class PermissionService {
  ...
  static async getUserPermissions(userId: string, tenantId: string): Promise<UserPermissions | null> {
    try {
      ...
      // For superusers and tenant owners, return all permissions
   ...
   */
  static shouldFilterToOwn(userPermissions: UserPermissions, resource: string): boolean {
  ...
  static async canEditTask(
    ...
    if (this.hasPermission(userPermissions, 'tasks.editOwn')) {
      ...
      if (userPermissions.personnelId &&
  ...
  static canAssignTasks(userPermissions: UserPermissions): boolean {
    return this.hasPermission(userPermissions, 'tasks.edit') ||
   ...
   */
  static async getUserPermissionContext(userId: string): Promise<UserPermissions & { isTenantOwner: boolean } | null> {
    try {
      const user = await User.findById(userId);
      if (!user) return null;
   ...
   */
  static async hasPermissionAsync(userId: string, permission: string, tenantId?: string): Promise<{ hasPermission: boolean; reason: string }> {
    try {
      const user = await User.findById(userId);
      if (!user) {
      ...
      // Superusers and tenant owners have all permissions
      if (user.role === 'superuser' || user.isTenantOwner) {
   ...
   */
  static async hasAnyPermission(userId: string, permissions: string[], tenantId?: string): Promise<{ hasPermission: boolean; reason: string }> {
    try {
      const user = await User.findById(userId);
      if (!user) {
        return { hasPermission: false, reason: 'User not found' };
   ...
   */
  static async hasAllPermissions(userId: string, permissions: string[], tenantId?: string): Promise<{ hasPermission: boolean; reason: string }> {
    try {
      const user = await User.findById(userId);
      if (!user) {
        return { hasPermission: false, reason: 'User not found' };
   ...
   */
  static async canAccessResource(userId: string, resource: string, action: string, tenantId?: string): Promise<{ hasPermission: boolean; reason: string }> {
